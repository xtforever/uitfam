...
HELP_MSG(SCAN, "SCAN <0-126> -- twi scan start" );
void cmd_SCAN(void)
{    
    u8 s=0,p=5;
    if( LN.buf[p-1] != 32 ) goto start_scanner;
    
    if( cl_num8(&p,&s) || s>126 ) {
	writeln("ERR SCAN parse arg: %s", LN.buf);
	return;
    }

 start_scanner:
    i2c_scanner_start(s);
    writeln("# SCAN started at %u", s ); 
}
